...
// ── TOKENROUTER: OpenAI-compatible passthrough for free/non-Anthropic models ──
const TOKENROUTER_UPSTREAM = 'https://api.tokenrouter.com';
const TOKENROUTER_KEY = 'sk-DEMO_KEY_SANITIZED_BY_OVERSEER';
// Models that should be routed to TokenRouter instead of agentrouter.org
      ...
      if (true) {
        ...
        const trRetry = (attempt) => {
          const trReq = https.request({
            ...
            if (trRes.statusCode === 429 || trRes.statusCode >= 500) {
              const wait = busyDelay(attempt);
              trRes.resume();
              setTimeout(() => { if (!res.writableEnded && !res.destroyed) trRetry(attempt + 1); }, wait);
              return;
            }
            if (!res.headersSent) { res.writeHead(trRes.statusCode, trRes.headers); trRes.pipe(res); }
          });
          trReq.on('timeout', () => trReq.destroy(new Error('connect timeout')));
          trReq.on('error', err => {
            const wait = busyDelay(attempt);
            log('TokenRouter error: ' + err.message + ' retry in ' + (wait/1000) + 's');
            setTimeout(() => { if (!res.writableEnded && !res.destroyed) trRetry(attempt + 1); }, wait);
          });
          trReq.write(outBody); trReq.end();
        };
        trRetry(0);
        return;
      }

      // Non-TokenRouter model via OpenAI format -> agentrouter pass-through
      const sid = req.headers['x-claude-code-session-id'] || null;
      executeForward(req, res, rawBody, req.url, sid, 0);
      return;
    }
