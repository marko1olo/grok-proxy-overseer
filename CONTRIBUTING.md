# 🛠️ Contributing to marko1olo/grok-proxy-overseer
...
> Technology Foundation: `Python 3.11 / FastAPI / aiohttp / Redis / Docker`
...
Grok Proxy Overseer LLM Model Proxy Controller is engineered for maximum performance, deterministic state transitions, and zero computational slop. All contributions must respect existing subsystem boundaries and data flows:

```mermaid
graph LR
    A[API Clients] --> B[FastAPI Proxy Gateway]
    B -->|Rate Limit Check| C[(Redis Sliding Window)]
    B -->|Authenticated Request| D[Upstream Grok / LLM API]
```
...
### 1. Deterministic Token Rate-Limiter
* **Formal Requirement**: Client quotas enforced via Redis sliding window algorithm.
...
### 2. Zero Key Exposure in Logs
* **Formal Requirement**: All bearer credentials masked before logging.
* **Verification Protocol**: Automated unit test assertion + mathematical boundary check.
* **Failure Mode**: Immediate build rejection; PR cannot be approved without meeting this invariant.
...
* Primary Runtime: `Python 3.11 / FastAPI / aiohttp / Redis / Docker`
...
git clone https://github.com/marko1olo/grok-proxy-overseer.git
cd grok-proxy-overseer
...
def sanitize_headers(headers: dict) -> dict:
    return {k: ('***' if k.lower() in ('authorization', 'x-api-key') else v) for k, v in headers.items()}
```
